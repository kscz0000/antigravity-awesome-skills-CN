...
# 用法: cmake -DCMAKE_BUILD_TYPE=ASAN ..
```
...
| Conan | 包管理器 | `conan install . --build=missing` |
...
| Catch2 | 单元测试 | `TEST_CASE("name") { REQUIRE(...); }` |
| GoogleTest | 单元测试 | `TEST(Suite, Name) { EXPECT_EQ(...); }` |
| Google Benchmark | 性能测试 | `BENCHMARK(func)->Range(...)` |
| Valgrind | 内存分析器 | `valgrind --tool=memcheck ./app` |
