---
name: fp-errors
description: 停止到处抛异常 - 使用 Either 和 TaskEither 将错误作为值处理，代码更清晰、更可预测。触发词：fp-ts 错误处理、Either、TaskEither、函数式错误处理、TypeScript 错误、验证、异常替代
risk: unknown
source: community
version: 1.0.0
author: kadu
...
本技能教你如何不用 try/catch 意大利面条式代码来处理错误。没有学术术语——只有解决实际问题的实用模式。

核心理念：**错误只是数据**。不要把它们抛入虚空然后希望有人捕获，而是将它们作为 TypeScript 可以追踪的值返回。

## 适用场景
- 需要用 `Either` 或 `TaskEither` 替换异常密集的代码。
- 任务涉及验证、领域错误或 TypeScript 中更清晰的错误契约。
- 需要针对实际应用代码的 fp-ts 错误处理实用指导。
...
fromNullable(user)  // 如果存在则 Right(user)，如果 null/undefined 则 Left('not found')
```
...
真正的威力在于链式调用。每一步都可能失败，但你把它写成清晰的管道。
...
真实代码库有抛异常的函数、可空值和 Promise。以下是如何与它们协作。
...
const user = await toThrowingPromise(fetchUser('123')) // 如果 Left 则抛异常
...
| 模式 | 使用场景 | 示例 |
|---------|----------|---------|
| `E.right(value)` | 创建成功值 | `E.right(42)` |
| `E.left(error)` | 创建失败值 | `E.left('not found')` |
...
| `E.filterOrElse(pred, onFalse)` | 带错误验证 | `E.filterOrElse(x => x > 0, () => 'must be positive')` |
| `sequenceS(validation)({...})` | 收集所有错误 | 表单验证 |

### TaskEither 等效操作

所有 Either 操作都有 TaskEither 等效版本：
...
1. **将错误作为值返回** - 使用 Either/TaskEither 而非抛异常
2. **自信地链式调用** - `chain` 自动在第一个错误处停止
3. **需要时收集所有错误** - 使用 validation applicative 处理表单
4. **在边界处包装** - 在边缘转换抛异常/Promise 代码
5. **最后再匹配** - 准备好行动时用 `fold` 处理两种情况
