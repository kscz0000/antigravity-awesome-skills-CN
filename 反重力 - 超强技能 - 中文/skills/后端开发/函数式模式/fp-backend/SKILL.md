---
name: fp-backend
description: Node.js/Deno 后端开发的函数式编程模式，使用 fp-ts、ReaderTaskEither 和函数式依赖注入。触发词：fp-ts 后端、函数式后端、ReaderTaskEither、函数式依赖注入、fp-ts 服务层、函数式架构、TypeScript 函数式编程、fp-ts 模式
risk: unknown
source: community
version: 1.0.0
author: kadu
...
- 任务涉及依赖注入、服务组合，或使用 `ReaderTaskEither` 进行类型化后端错误处理。
- 你需要函数式后端架构模式，而非孤立的工具代码片段。

## 核心概念

### ReaderTaskEither (RTE)

`ReaderTaskEither<R, E, A>` 类型是函数式后端开发的基石：
...
| `RTE.left(e)` | 创建错误 |
| `RTE.ask<R>()` | 获取依赖 |
| `RTE.fromTaskEither(te)` | 提升 TaskEither |
| `RTE.fromEither(e)` | 提升 Either |
| `RTE.fromOption(onNone)(o)` | 提升 Option |
| `RTE.flatMap(f)` | 链式操作 |
| `RTE.map(f)` | 转换成功值 |
| `RTE.mapLeft(f)` | 转换错误 |
| `RTE.tap(f)` | 成功时执行副作用 |
| `RTE.tapError(f)` | 错误时执行副作用 |
| `RTE.orElse(f)` | 从错误中恢复 |
| `RTE.getOrElse(f)` | 提取并提供回退值 |
