---
name: fp-ts-react
description: "fp-ts 与 React 结合的实用模式——hooks、状态、表单、数据获取。使用 React 18/19、Next.js 14/15 构建函数式编程风格的 React 应用时使用。触发词：fp-ts React、函数式 React、Option 状态、Either 表单验证、TaskEither 数据获取、RemoteData 模式、React 函数式编程、fp-ts hooks"
risk: safe
...
React 应用的实用模式。不谈术语，只写能用的代码。

## 何时使用此技能

- 使用 fp-ts 构建 React 应用，实现类型安全的状态管理
- 处理数据获取中的加载/错误/成功状态
- 实现带错误累积的表单验证
- 在 React 18/19 或 Next.js 14/15 中使用函数式模式
...
使用 `Option` 替代 `null | undefined`，意图更清晰。
...
// 验证函数返回 Either<ErrorMessage, ValidValue>
...
// 可能出现：{ data: user, loading: true, error: someError }——这代表什么？！
...
像 `O.some(1)` 这样的 fp-ts 值每次渲染都会创建新对象。React 会认为它们"改变了"。
...
| 多字段表单验证 | `Either` 配合验证 applicative |
| 依赖注入 | Context + `ReaderTaskEither` |
| 防止 fp-ts 导致的重渲染 | `useMemo` 或 `fp-ts-react-stable-hooks` |
