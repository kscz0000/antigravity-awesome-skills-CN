---
name: transitions
description: "Remotion 的全屏场景过渡。触发词：transitions、fade、slide、wipe、scenes、remotion"
metadata:
...
使用 `<TransitionSeries>` 在多个场景或剪辑之间进行动画。
这将绝对定位子元素。
...
过渡会重叠相邻场景，因此合成总长度**短于**所有序列时长的总和。
...
对于没有显式 `durationInFrames` 的 `springTiming`，时长取决于 `fps`，因为它计算弹簧动画何时稳定。
