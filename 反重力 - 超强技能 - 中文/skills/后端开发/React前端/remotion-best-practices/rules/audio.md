---
name: audio
description: "在 Remotion 中使用音频和声音 - 导入、剪辑、音量、速度、音调。触发词：audio、media、trim、volume、speed、loop、pitch、mute、sound、sfx、remotion"
metadata:
...
使用来自 `@remotion/media` 的 `<Audio>` 组件向合成添加音频。
...
默认情况下，音频从开头播放，音量最大，完整长度。
可以通过添加多个 `<Audio>` 组件来叠加多个音频轨道。
...
音调偏移仅在服务器端渲染期间有效，在 Remotion Studio 预览或 `<Player />` 中无效。