# Godot GDScript 模式实施手册

本文件包含技能引用的详细模式、检查清单和代码示例。

# Godot GDScript 模式

Godot 4.x 游戏开发的 GDScript 生产级模式，涵盖架构、信号、场景和优化。
...
- **使用信号解耦** - 避免直接引用
- **所有变量加类型** - 静态类型能捕获错误
- **用 Resource 管理数据** - 将数据与逻辑分离
- **对频繁生成的对象使用对象池** - 避免 GC 卡顿
- **谨慎使用 Autoload** - 仅用于真正的全局系统

### 避免做法
- **不要在循环中使用 `get_node()`** - 缓存引用
- **不要紧耦合场景** - 使用信号
- **不要在 Resource 中放逻辑** - 保持纯数据
- **不要忽视性能分析器** - 监控性能
- **不要对抗场景树** - 遵循 Godot 的设计
