...
综合运用所有现代模式的完整工作示例：React.FC、懒加载、Suspense、useSuspenseQuery、样式、路由和错误处理。

---

## 示例 1：完整现代组件

组合使用：React.FC、useSuspenseQuery、缓存优先、useCallback、样式、错误处理
...
2. **功能结构**：按子目录组织（api/、components/、hooks/ 等）
3. **路由**：基于文件夹的路由 + 懒加载
4. **数据获取**：useSuspenseQuery + 缓存优先策略
5. **表单**：React Hook Blog + Zod 验证
6. **错误处理**：useMuiSnackbar + onError 回调
7. **性能优化**：useMemo、useCallback、React.memo、防抖
8. **样式**：100 行以内使用内联样式、sx prop、MUI v7 语法
