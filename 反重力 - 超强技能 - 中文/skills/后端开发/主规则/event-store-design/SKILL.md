...
description: "为事件溯源系统设计和实现事件存储。适用于构建事件溯源基础设施、选择事件存储技术或实现事件持久化模式。"
risk: unknown
source: community
...
| **PostgreSQL**   | 现有 Postgres 技术栈   | 需手动实现            |
| **Kafka**        | 高吞吐量流式处理 | 不适合流级查询 |
