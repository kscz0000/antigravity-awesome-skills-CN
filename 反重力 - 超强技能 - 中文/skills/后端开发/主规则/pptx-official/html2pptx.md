...
- Flexbox 可用 - 位置从渲染的布局计算
- CSS 中使用带 `#` 前缀的十六进制颜色
- **文本对齐**：需要时使用 CSS `text-align`（`center`、`right` 等）作为 PptxGenJS 文本格式化的提示，以应对文本长度略有偏差的情况

### 形状样式（仅限 DIV 元素）

**重要：背景、边框和阴影仅适用于 `<div>` 元素，不适用于文本元素（`<p>`、`<h1>`-`<h6>`、`<ul>`、`<ol>`）**

- **背景**：CSS `background` 或 `background-color` 仅适用于 `<div>` 元素
...
- **圆角**：CSS `border-radius` 在 `<div>` 元素上创建圆角
  - `border-radius: 50%` 或更高创建圆形
  - 百分比 <50% 相对于形状较小尺寸计算
  - 支持 px 和 pt 单位（如 `border-radius: 8pt;`、`border-radius: 12px;`）
  - 示例：`<div style="border-radius: 25%;">` 在 100x200px 的框上 = 100px 的 25% = 25px 半径
- **盒阴影**：CSS `box-shadow` 在 `<div>` 元素上转换为 PowerPoint 阴影
  - 仅支持外部阴影（inset 阴影被忽略以防止损坏）
  - 示例：`<div style="box-shadow: 2px 2px 8px rgba(0, 0, 0, 0.3);">`
  - 注意：PowerPoint 不支持 inset/inner 阴影，将被跳过

### 图标与渐变

- **关键：切勿使用 CSS 渐变（`linear-gradient`、`radial-gradient`）** - 它们无法转换为 PowerPoint
- **始终先使用 Sharp 创建渐变/图标 PNG，然后在 HTML 中引用**
- 渐变：将 SVG 栅格化为 PNG 背景图片
- 图标：将 react-icons SVG 栅格化为 PNG 图片
- 所有视觉效果必须在 HTML 渲染前预渲染为光栅图片
...
**所有验证错误会一起收集并报告**在单个错误消息中，允许你一次性修复所有问题，而不是逐个修复。
...
使用 `html2pptx` 将 HTML 转换为幻灯片后，你将使用 PptxGenJS 添加动态内容，如图表、图片和其他元素。

### ⚠️ 关键规则

#### 颜色
- **在 PptxGenJS 中切勿使用 `#` 前缀**的十六进制颜色 - 会导致文件损坏
...
**大多数图表需要：** 使用 `catAxisTitle`（类别）和 `valAxisTitle`（值）设置坐标轴标签。

**图表数据格式：**
- 对于简单的条形图/折线图，使用**包含所有标签的单系列**
- 每个系列创建单独的图例条目
- labels 数组定义 X 轴值

**时间序列数据 - 选择正确的粒度：**
- **< 30 天**：使用按日分组（如 "10-01"、"10-02"）- 避免按月聚合导致单点图表
- **30-365 天**：使用按月分组（如 "2024-01"、"2024-02"）
- **> 365 天**：使用按年分组（如 "2023"、"2024"）
- **验证**：仅有 1 个数据点的图表可能表明时间周期的聚合不正确
...
**将图表颜色与你选择的设计配色方案对齐**，确保数据可视化有足够的对比度和区分度。调整颜色以：
- 相邻系列之间有强对比度
- 在幻灯片背景上可读
- 无障碍性（避免仅红绿组合）
