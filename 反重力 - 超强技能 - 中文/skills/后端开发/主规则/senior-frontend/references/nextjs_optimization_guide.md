...
服务端组件在服务器上渲染并将 HTML 发送至客户端。适用于数据密集型、非交互式内容。
...
| 领域 | 优化措施 | 影响 |
|------|---------|------|
| 图片 | 使用 next/image 并为 LCP 设置 priority | 高 |
| 字体 | 使用 next/font 并设置 display: swap | 中 |
| 代码 | 对重型组件使用动态导入 | 高 |
| 数据 | 使用 Promise.all 并行获取 | 高 |
| 渲染 | 默认使用服务端组件 | 高 |
| 缓存 | 合理配置 revalidate | 中 |
| 包体积 | Tree-shake 导入，分析体积 | 中 |
