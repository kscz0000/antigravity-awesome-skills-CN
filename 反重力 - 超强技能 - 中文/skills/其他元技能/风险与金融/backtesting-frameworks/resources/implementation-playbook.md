...
| 偏差 | 描述 | 缓解措施 |
|------|------|----------|
| **前视偏差 (Look-ahead)** | 使用未来信息 | 使用时点数据 |
| **幸存者偏差 (Survivorship)** | 仅在幸存标的上测试 | 包含已退市证券 |
| **过拟合 (Overfitting)** | 对历史数据曲线拟合 | 样本外测试 |
| **选择偏差 (Selection)** | 精挑细选策略 | 预注册 |
| **交易偏差 (Transaction)** | 忽略交易成本 | 真实成本模型 |
...
- **包含交易成本** - 真实估算
- **样本外测试** - 始终预留数据
- **使用 Walk-Forward** - 不只是训练/测试
- **Monte Carlo 分析** - 理解不确定性
