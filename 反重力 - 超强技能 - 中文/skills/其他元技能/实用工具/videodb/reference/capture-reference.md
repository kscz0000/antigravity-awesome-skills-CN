...
VideoDB 捕获会话的代码级详情。工作流指南见 [capture.md](capture.md)。
...
使用 [scripts/ws_listener.py](../scripts/ws_listener.py) 连接并将事件转储到 `/tmp/videodb_events.jsonl`。
...
> 最新详情见 https://docs.videodb.io/pages/ingest/capture-sdks/realtime-context.md
...
| `ws.receive()` | `AsyncIterator[dict]` | 产生实时消息的异步迭代器 |
...
| `conn.generate_client_token()` | `str` | 生成客户端认证令牌 |
...
> **注意：** `end_user_id` 是必需的，用于标识发起捕获的用户。用于测试或演示时，任何唯一字符串标识符都可以（如 `"demo-user"`、`"test-123"`）。
...
| `await client.stop_capture()` | `None` | 优雅停止捕获会话 |
...
| `channels.displays` | `ChannelGroup` | 可用的屏幕显示 |
...
关于 RTStream 方法（索引、转录、警报、批配置），见 [rtstream-reference.md](rtstream-reference.md)。
