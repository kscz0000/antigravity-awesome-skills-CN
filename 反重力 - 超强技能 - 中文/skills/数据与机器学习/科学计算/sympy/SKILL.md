---
name: sympy
description: "SymPy 是 Python 符号数学库，支持使用数学符号进行精确计算而非数值近似。当用户需要进行符号代数、微积分、方程求解、矩阵运算、物理计算或代码生成时使用。触发词：符号计算、符号数学、SymPy、方程求解、微积分、矩阵符号运算、符号代数。"
...
SymPy 是一个用于符号数学的 Python 库，支持使用数学符号进行精确计算而非数值近似。本技能提供使用 SymPy 进行符号代数、微积分、线性代数、方程求解、物理计算和代码生成的全面指导。
...
**完整线性代数内容：** 参见 `references/matrices-linear-algebra.md`
...
- **组合数学：** 排列、组合、分拆、群论
- **逻辑和集合：** 布尔逻辑、集合论、有限集和无限集
- **统计学：** 概率分布、随机变量、期望、方差
- **特殊函数：** Gamma 函数、Bessel 函数、正交多项式、超几何函数
...
**完整代码生成内容：** 参见 `references/code-generation-printing.md`
...
print(f"美观打印: {pretty(integral_expr)} = {pretty(result)}")
...
1. **"NameError: name 'x' is not defined"**
   - 解决方案：使用前始终用 `symbols()` 定义符号

2. **意外的数值结果**
   - 问题：使用浮点数如 `0.5` 而非 `Rational(1, 2)`
...
5. **简化未按预期工作**
   ...
   - 为符号添加假设条件（如 `positive=True`）
   - 使用 `simplify(expr, force=True)` 进行强制简化
