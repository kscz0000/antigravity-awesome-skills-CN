---
name: biopython
description: "Biopython 是一套免费可用的 Python 生物计算工具集。提供序列操作、文件 I/O、数据库访问、结构生物信息学、系统发育学等功能。触发词：Biopython、生物序列、DNA序列、RNA序列、蛋白质序列、FASTA、GenBank、BLAST、PDB、系统发育树、序列比对、NCBI、Entrez、分子生物学、生物信息学"
license: Unknown
metadata:
...
2. **比对分析** - Bio.Align 和 Bio.AlignIO 用于双序列和多序列比对
...
5. **对大文件使用迭代器** - 避免将所有内容加载到内存
   ```python
   for record in SeqIO.parse("large_file.fasta", "fasta"):
       # 一次处理一条记录
   ```

6. **优雅地处理错误** - 网络操作和文件解析可能失败
   ```python
   try:
...
1. **识别任务领域**（序列、比对、数据库、BLAST、结构、系统发育或高级）
2. **查阅适当的参考文件** 在 `references/` 目录中
3. **调整代码示例** 以适应特定用例
4. **组合多个模块** 当复杂工作流需要时
