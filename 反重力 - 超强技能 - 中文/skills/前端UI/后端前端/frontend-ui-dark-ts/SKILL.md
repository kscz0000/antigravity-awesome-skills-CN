...
description: "现代深色主题 React UI 系统，使用 Tailwind CSS 和 Framer Motion。适用于仪表盘、管理后台和数据密集型应用，具有毛玻璃效果和精致的动画。当用户要求'深色主题UI'、'暗色仪表盘'、'glassmorphism界面'时使用。"
risk: unknown
source: community
...
现代深色主题 React UI 系统，使用 **Tailwind CSS** 和 **Framer Motion**。专为仪表盘、管理后台和数据密集型应用设计，具有毛玻璃效果和精致的动画。
...
```
public/
├── favicon.ico                    # 经典图标 (32x32)
├── favicon.svg                    # 现代 SVG 图标
...
| 元素 | 类名 |
|---------|---------|
| 页面标题 | `text-2xl font-semibold text-text-primary` |
| 章节标题 | `text-lg font-semibold text-text-primary` |
| 卡片标题 | `text-base font-medium text-text-primary` |
| 正文文本 | `text-sm text-text-secondary` |
| 说明文字 | `text-xs text-text-muted` |
...
| 用途 | 颜色 | 类名 |
|----------|-------|-------|
| 主要操作 | 品牌紫色 | `bg-brand text-white` |
| 主要悬停 | 品牌悬停色 | `hover:bg-brand-hover` |
| 页面背景 | 中性 bg1 | `bg-neutral-bg1` |
| 卡片背景 | 中性 bg2 | `bg-neutral-bg2` |
| 抬升表面 | 中性 bg3 | `bg-neutral-bg3` |
| 输入框背景 | 中性 bg2 | `bg-neutral-bg2` |
| 输入框聚焦 | 中性 bg3 | `focus:bg-neutral-bg3` |
| 默认边框 | 边框默认 | `border-border` |
| 淡边框 | 边框淡色 | `border-border-subtle` |
| 成功 | 状态成功 | `text-status-success` |
| 警告 | 状态警告 | `text-status-warning` |
| 错误 | 状态错误 | `text-status-error` |

## 相关文件

- Design Tokens — 完整的颜色系统、间距、排版比例
- Components — Button、Card、Input、Dialog、Tabs 等
- Patterns — 页面布局、导航、列表、表单
