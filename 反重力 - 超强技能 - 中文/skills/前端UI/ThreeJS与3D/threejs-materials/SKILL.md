...
description: Three.js 材质系统 — PBR、基础、Phong、Shader 材质及材质属性。适用于网格样式、纹理处理、自定义着色器开发或材质性能优化。触发词：Three.js 材质、PBR 材质、着色器、ShaderMaterial、材质属性、网格材质
risk: unknown
source: community
---
...
| MeshPhongMaterial    | 光泽表面、高光反射                | 是                 |
| MeshStandardMaterial | PBR、写实材质                     | 是（PBR）          |
| MeshPhysicalMaterial | 高级 PBR、清漆、透射              | 是（PBR+）         |
| MeshToonMaterial     | 卡通渲染、赛璐珞风格              | 是（卡通）         |
| MeshNormalMaterial   | 法线调试                          | 否                 |
...
1. **复用材质**：相同材质 = 合并绘制调用
2. **尽量避免透明**：透明材质需要排序
3. **用 alphaTest 替代透明**：适用时性能更好
4. **选择更简单的材质**：Basic > Lambert > Phong > Standard > Physical
5. **限制活跃光源数量**：每个光源增加着色器复杂度
...
## NodeMaterial / TSL（未来方向）

Three.js 正在将 **NodeMaterial** 和 **TSL（Three.js Shading Language）** 作为标准材质系统推进，尤其面向 WebGPU 渲染器：
...
**要点：**
- NodeMaterial 同时兼容 WebGL 和 WebGPU 渲染器
- `onBeforeCompile` 在 WebGPU 渲染器下**不可用** — 请改用 NodeMaterial
- TSL 替代 GLSL，实现跨渲染器着色器兼容
- 标准 GLSL `ShaderMaterial` 在 WebGL 渲染器下继续可用

## Lambert/Phong IBL 支持（r183）

从 r183 起，`MeshLambertMaterial` 和 `MeshPhongMaterial` 支持通过 `scene.environment` 进行基于图像的光照（IBL）。此前只有 PBR 材质（Standard/Physical）能响应场景级环境贴图。

## 相关技能

- `threejs-textures` — 纹理加载与配置
- `threejs-shaders` — 自定义着色器开发
- `threejs-lighting` — 光照与材质交互
