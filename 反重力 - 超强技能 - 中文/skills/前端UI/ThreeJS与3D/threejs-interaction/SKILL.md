...
description: Three.js 交互功能——射线检测、控制器、鼠标/触摸输入、物体选取。用于处理用户输入、实现点击检测、添加相机控制或创建交互式 3D 体验。触发词：交互、射线检测、点击检测、鼠标输入、触摸输入、相机控制、物体选取、拖拽、hover 效果。
risk: unknown
source: community
---
...
## 适用场景
- 需要在 Three.js 场景中实现用户交互。
- 任务涉及射线检测、物体拾取、指针处理、触摸输入或相机控制。
- 构建的是交互式 3D 体验，而非被动渲染。
...
1. **限制射线检测频率**：节流 mousemove 处理函数
2. **使用 Layers**：过滤射线检测目标
3. **简化碰撞网格**：用不可见的简单几何体做射线检测
...
- `threejs-fundamentals` - 相机与场景搭建
- `threejs-animation` - 交互动画
- `threejs-shaders` - 视觉反馈特效
