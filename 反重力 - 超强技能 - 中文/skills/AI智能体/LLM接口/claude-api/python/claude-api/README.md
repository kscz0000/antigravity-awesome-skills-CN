...
使用顶层 `cache_control` 自动缓存请求中最后一个可缓存的块——无需注释单个内容块：
...
> **Beta，仅限 Opus 4.6。** 当对话接近 200K 上下文窗口时，压缩会在服务端自动总结较早的上下文。API 返回一个 `compaction` 块；你必须在后续请求中将其传回——追加 `response.content`，不只是文本。
...
# 大多数任务默认使用 Opus
response = client.messages.create(
    model="claude-opus-4-6",  # $5.00/$25.00 每 1M tokens
    max_tokens=1024,
...
standard_response = client.messages.create(
    model="claude-sonnet-4-6",  # $3.00/$15.00 每 1M tokens
    max_tokens=1024,
...
simple_response = client.messages.create(
    model="claude-haiku-4-5",  # $1.00/$5.00 每 1M tokens
    max_tokens=256,
...
> **注意：** Anthropic SDK 自动使用指数退避重试速率限制（429）和服务器错误（5xx）。你可以通过 `max_retries` 配置（默认：2）。仅当你需要 SDK 提供之外的行为时才实现自定义重试逻辑。
