...
description: "使用 Hugging Face Hub CLI (`hf`) 下载、上传和管理模型、数据集及 Spaces。"
risk: unknown
---
...
适用于需要 `hf` CLI 进行 Hub 认证、下载、上传、仓库管理或基础计算操作的场景。

Hugging Face Hub CLI 工具 `hf` 已可用。重要提示：`hf` 命令替代了已弃用的 `huggingface-cli` 命令。

使用 `hf --help` 查看可用功能。注意认证命令现在都在 `hf auth` 下，例如 `hf auth whoami`。

基于 `huggingface_hub v1.8.0` 生成。运行 `hf skills add --force` 重新生成。
...
- `hf env` — 打印环境信息。
- `hf sync` — 同步本地目录与存储桶之间的文件。`[--delete --ignore-times --ignore-sizes --plan TEXT --apply TEXT --dry-run --include TEXT --exclude TEXT --filter-from TEXT --existing --ignore-existing --verbose --quiet]`
- `hf upload REPO_ID` — 上传文件或文件夹到 Hub。适用于单次提交上传。`[--type CHOICE --revision TEXT --private --include TEXT --exclude TEXT --delete TEXT --commit-message TEXT --commit-description TEXT --create-pr --every FLOAT --quiet]`
- `hf upload-large-folder REPO_ID LOCAL_PATH` — 上传大型文件夹到 Hub。适用于可恢复上传。`[--type CHOICE --revision TEXT --private --include TEXT --exclude TEXT --num-workers INTEGER --no-report --no-bars]`
- `hf version` — 打印 hf 版本信息。

### `hf auth` — 管理认证（登录、登出等）。

- `hf auth list` — 列出所有已存储的访问令牌。
- `hf auth login` — 使用 nuggingface.co/settings/tokens 的令牌登录。`[--add-to-git-credential --force]`
- `hf auth logout` — 登出指定令牌。`[--token-name TEXT]`
- `hf auth switch` — 在访问令牌之间切换。`[--token-name TEXT --add-to-git-credential]`
- `hf auth whoami` — 查看当前登录的 huggingface.co 账户。`[--format CHOICE]`

### `hf buckets` — 与存储桶交互的命令。

- `hf buckets cp SRC` — 复制单个文件到或从存储桶。`[--quiet]`
...
- `hf buckets list` — 列出存储桶或桶内文件。`[--human-readable --tree --recursive --format CHOICE --quiet]`
- `hf buckets move FROM_ID TO_ID` — 移动（重命名）存储桶到新名称或命名空间。
- `hf buckets remove ARGUMENT` — 从存储桶中删除文件。`[--recursive --yes --dry-run --include TEXT --exclude TEXT --quiet]`
- `hf buckets sync` — 同步本地目录与存储桶之间的文件。`[--delete --ignore-times --ignore-sizes --plan TEXT --apply TEXT --dry-run --include TEXT --exclude TEXT --filter-from TEXT --existing --ignore-existing --verbose --quiet]`

### `hf cache` — 管理本地缓存目录。

- `hf cache list` — 列出缓存的仓库或版本。`[--cache-dir TEXT --revisions --filter TEXT --format CHOICE --quiet --sort CHOICE --limit INTEGER]`
- `hf cache prune` — 从缓存中移除孤立版本。`[--cache-dir TEXT --yes --dry-run]`
- `hf cache rm TARGETS` — 移除缓存的仓库或版本。`[--cache-dir TEXT --yes --dry-run]`
- `hf cache verify REPO_ID` — 验证缓存或本地目录中单个仓库版本的校验和。`[--type CHOICE --revision TEXT --cache-dir TEXT --local-dir TEXT --fail-on-missing-files --fail-on-extra-files]`
...
- `hf discussions comment REPO_ID NUM` — 在讨论或拉取请求中评论。`[--body TEXT --body-file PATH --type CHOICE]`
- `hf discussions create REPO_ID --title TEXT` — 在仓库中创建新讨论或拉取请求。`[--body TEXT --body-file PATH --pull-request --type CHOICE]`
...
- `hf discussions list REPO_ID` — 列出仓库中的讨论和拉取请求。`[--status CHOICE --kind CHOICE --author TEXT --limit INTEGER --type CHOICE --format CHOICE --quiet]`
...
- `hf endpoints list` — 列出指定命名空间的所有推理端点。`[--namespace TEXT --format CHOICE --quiet]`
- `hf endpoints pause NAME` — 暂停推理端点。`[--namespace TEXT]`
- `hf endpoints resume NAME` — 恢复推理端点。`[--namespace TEXT --fail-if-already-running]`
- `hf endpoints scale-to-zero NAME` — 将推理端点缩容至零。`[--namespace TEXT]`
...
- `hf extensions install REPO_ID` — 从公开 GitHub 仓库安装扩展。`[--force]`
...
- `hf jobs logs JOB_ID` — 获取作业日志。`[--follow --tail INTEGER --namespace TEXT]`
...
- `hf jobs scheduled suspend SCHEDULED_JOB_ID` — 暂停计划作业。`[--namespace TEXT]`
...
- `hf repos delete REPO_ID` — 从 Hub 删除仓库。此操作不可逆。`[--type CHOICE --missing-ok]`
...
- `hf repos duplicate FROM_ID` — 复制 Hub 上的仓库（模型、数据集或 Space）。`[--type CHOICE --private --public --protected --exist-ok --flavor TEXT --storage TEXT --sleep-time INTEGER --secrets TEXT --secrets-file TEXT --env TEXT --env-file TEXT]`
- `hf repos move FROM_ID TO_ID` — 将仓库从一个命名空间移动到另一个命名空间。`[--type CHOICE]`
- `hf repos settings REPO_ID` — 更新仓库设置。`[--gated CHOICE --private --public --protected --type CHOICE]`
- `hf repos tag create REPO_ID TAG` — 为仓库创建标签。`[--message TEXT --revision TEXT --type CHOICE]`
- `hf repos tag delete REPO_ID TAG` — 删除仓库标签。`[--yes --type CHOICE]`
- `hf repos tag list REPO_ID` — 列出仓库标签。`[--type CHOICE]`
...
- `hf spaces dev-mode SPACE_ID` — 启用或禁用 Space 的开发模式。`[--stop]`
- `hf spaces hot-reload SPACE_ID` — 无需完整重建和重启即可热重载 Space 的任意 Python 文件。`[--local-file TEXT --skip-checks --skip-summary]`
...
- `hf webhooks disable WEBHOOK_ID` — 禁用活跃的 webhook。
- `hf webhooks enable WEBHOOK_ID` — 启用已禁用的 webhook。
- `hf webhooks info WEBHOOK_ID` — 以 JSON 格式显示单个 webhook 的完整详情。
...
- `-q / --quiet` — 最小化输出。
- `--revision` — Git 版本 ID，可以是分支名、标签或提交哈希。
- `--token` — 使用用户访问令牌。建议设置 `HF_TOKEN` 环境变量而非传递 `--token`。
...
- 使用 `hf <command> --help` 查看完整选项、描述、用法和实际示例
- 使用 `HF_TOKEN` 环境变量认证（推荐）或使用 `--token`
