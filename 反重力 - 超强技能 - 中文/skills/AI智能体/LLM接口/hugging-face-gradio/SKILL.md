...
description: 构建或编辑 Gradio 应用、布局、组件和聊天界面（Python）。
risk: unknown
---
...
## 适用场景
适用于用户需要 Gradio 演示、UI 原型或基于 Python 的机器学习界面。

Gradio 是一个用于构建交互式 Web UI 和机器学习演示的 Python 库。本技能涵盖核心 API、模式和示例。
...
- [分享应用](https://www.gradio.app/guides/sharing-your-app)
...
创建从 {minimum} 到 {maximum} 范围的滑块，步长为 {step}。
...
创建下拉选择框，可选择单个或多个条目（作为输入组件）或显示条目（作为输出组件）。
...
创建音频组件，用于上传/录制音频（作为输入）或播放音频（作为输出）。
...
创建视频组件，用于上传/录制视频（作为输入）或播放视频（作为输出）。
...
创建文件组件，用于上传一个或多个通用文件（作为输入）或显示文件/URL供下载（作为输出）。
...
如果任务需要对现有组件进行大幅定制，或需要 Gradio 中不存在的组件，可以使用 `gr.HTML` 创建。它支持 `html_template`（使用 `${}` JS 表达式和 `{{}}` Handlebars 语法）、`css_template` 用于作用域样式，以及 `js_on_load` 用于交互——其中 `props.value` 更新组件值，`trigger('event_name')` 触发 Gradio 事件。如需复用，可继承 `gr.HTML` 并定义 `api_info()` 以支持 API/MCP。详见[完整指南](https://www.gradio.app/guides/custom-HTML-components)。
...
`gradio` CLI 包含 `info` 和 `predict` 命令，用于以编程方式与 Gradio 应用交互。这对需要在工作流中使用 Spaces 的编码智能体特别有用。
...
返回描述所有端点的 JSON 载荷，包括参数（类型和默认值）及返回值。
...
文件类型参数显示 `"type": "filepath"`，并提示包含 `"meta": {"_type": "gradio.FileData"}`——这表示文件将被上传到远程服务器。

### `gradio predict` — 发送预测请求
...
两个命令都支持 `--token` 参数以访问私有 Spaces。

## 其他参考

- [端到端示例](examples.md) — 完整可运行的应用
