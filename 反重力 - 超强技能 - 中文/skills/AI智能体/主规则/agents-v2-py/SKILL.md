---
name: agents-v2-py
description: "使用 Azure AI Projects SDK 构建基于容器的 Foundry Agents（ImageBasedHostedAgentDefinition）。当创建使用自定义容器镜像的 Azure AI Foundry 托管代理时使用。触发词：Azure AI 托管代理、Foundry Agents、容器化代理、ImageBasedHostedAgentDefinition、Azure AI Projects SDK、托管智能体、容器镜像代理"
risk: unknown
source: community
...
3. **能力宿主** - 账户级别的能力宿主，需启用 `enablePublicHostingEnvironment=true`
4. **SDK 版本** - 确保安装 `azure-ai-projects>=2.0.0b3`
...
| `CapabilityHostNotFound` | 未配置能力宿主 | 创建账户级别的能力宿主 |
| `ProtocolVersionNotSupported` | 协议版本无效 | 使用 `AgentProtocol.RESPONSES` 和版本 `"v1"` |

## 最佳实践

1. **镜像版本化** - 生产环境中使用具体标签，而非 `latest`
2. **最小资源** - 从最小 CPU/内存开始，按需扩展
3. **环境变量** - 用于所有配置，切勿硬编码
4. **错误处理** - 用 try/except 块包装代理创建逻辑
5. **清理** - 删除未使用的代理版本以释放资源
