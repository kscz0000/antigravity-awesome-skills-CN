# Transformers.js 代码示例

展示在不同运行时和框架中使用 Transformers.js 的实际示例。

所有示例使用相同的任务和模型以保持一致性：
- **任务**: `feature-extraction`
- **模型**: `onnx-community/all-MiniLM-L6-v2-ONNX`

## 目录
1. [浏览器（原生 JS）](#浏览器原生-js)
2. [Node.js](#nodejs)
3. [React](#react)
...
这些示例展示了相同功能在不同运行时和框架中的实现方式，便于适配你的具体场景。所有示例都包含 `.dispose()` 清理以释放内存。
