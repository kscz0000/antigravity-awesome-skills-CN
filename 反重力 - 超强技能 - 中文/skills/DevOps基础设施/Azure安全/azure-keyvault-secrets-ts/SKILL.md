...
description: "使用 Azure Key Vault Secrets SDK for JavaScript (@azure/keyvault-secrets) 管理机密。适用于存储和检索应用程序机密或配置值。当用户要求'管理 Azure Key Vault 机密'、'存储和检索机密'、'Azure 密钥保管库操作'时使用。"
risk: unknown
source: community
...
2. **启用软删除** - 生产环境保管库必须启用
3. **设置过期日期** - 同时为 keys 和 secrets 设置
4. **使用密钥轮换策略** - 自动化密钥轮换
