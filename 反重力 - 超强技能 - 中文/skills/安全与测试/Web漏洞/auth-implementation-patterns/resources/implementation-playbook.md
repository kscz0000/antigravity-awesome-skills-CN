...
1. **永不存储明文密码**：始终使用 bcrypt/argon2 哈希
2. **使用 HTTPS**：加密传输中的数据
3. **短期访问令牌**：最长 15-30 分钟
4. **安全 Cookie**：httpOnly、secure、sameSite 标志
5. **验证所有输入**：邮箱格式、密码强度
6. **对认证端点进行速率限制**：防止暴力攻击
7. **实现 CSRF 保护**：针对基于会话的认证
...
- **弱密码**：强制执行强密码策略
- **JWT 存储在 localStorage**：易受 XSS 攻击，使用 httpOnly Cookie
- **无令牌过期**：令牌必须过期
- **仅客户端认证检查**：始终在服务器端验证
...
- **references/jwt-best-practices.md**：JWT 实现指南
