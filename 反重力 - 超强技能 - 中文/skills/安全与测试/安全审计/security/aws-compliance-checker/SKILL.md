...
description: "自动化检查 CIS、PCI-DSS、HIPAA、SOC 2 等行业基准的合规情况。涉及 AWS 合规检查、审计准备、合规监控、PCI-DSS 报告、HIPAA 审计、CIS 基准、AWS Config、合规自动化时使用。"
category: security
risk: safe
source: community
...
针对 CIS AWS Foundations、PCI-DSS、HIPAA、SOC 2 等行业标准进行自动化合规校验。

## 适用场景

需要按照行业标准校验 AWS 合规情况、准备审计，或保持持续合规监控时使用本技能。

## 支持的合规框架

**CIS AWS Foundations 基准**
- 身份与访问管理
- 日志与监控
- 网络
- 数据保护
...
- "运行 CIS AWS Foundations 合规检查"
- "生成 PCI-DSS 合规报告"
- "检查我的 AWS 账户的 HIPAA 合规情况"
- "按 SOC 2 要求进行审计"
- "创建一个合规仪表盘"
