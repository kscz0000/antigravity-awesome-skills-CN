# 使用 Mock Activity 进行集成测试

使用 mock 外部依赖、错误注入和复杂场景测试工作流的全面模式指南。

## Activity Mock 策略

**目的**：测试工作流编排逻辑，无需调用真实外部服务
...
1. **Mock 外部依赖**：测试中禁止调用真实 API
2. **测试错误场景**：验证补偿和重试逻辑
3. **并行测试**：使用 pytest-xdist 加速测试运行
4. **测试隔离**：每个测试应相互独立
5. **清晰断言**：同时验证结果和副作用
6. **覆盖率目标**：关键工作流 ≥80%
7. **快速执行**：使用时间跳过，避免真实延迟
