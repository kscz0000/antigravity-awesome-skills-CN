...
description: "分析编译后二进制文件、理解汇编代码和重建程序逻辑的综合模式与技术。当用户要求'二进制分析'、'反汇编模式'、'逆向工程'、'汇编代码分析'、'二进制逆向'时使用。"
risk: unknown
source: community
...
分析编译后二进制文件、理解汇编代码和重建程序逻辑的综合模式与技术。

## 使用此技能的场景

- 从事二进制分析模式相关任务或工作流
- 需要二进制分析模式的指导、最佳实践或检查清单
...
2. **字符串分析**：识别有价值的字符串和错误消息
3. **函数识别**：入口点、导出、交叉引用
4. **控制流映射**：理解程序结构
5. **数据结构恢复**：识别结构体、数组、全局变量
6. **算法识别**：加密、哈希、压缩
7. **文档记录**：注释、重命名符号、类型定义

### 常见陷阱

- **优化器产物**：代码可能与源码结构不匹配
- **内联函数**：函数可能被内联展开
- **尾调用优化**：用 `jmp` 代替 `call` + `ret`
- **死代码**：优化后不可达的代码
- **位置无关代码**：RIP 相对寻址
