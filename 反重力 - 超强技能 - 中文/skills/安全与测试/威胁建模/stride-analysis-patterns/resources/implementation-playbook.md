...
| 分类 | 问题 | 控制领域 |
|------|------|----------|
| **Spoofing（仿冒）** | 攻击者能否冒充他人身份？ | 身份认证 |
| **Tampering（篡改）** | 攻击者能否修改传输中或静态数据？ | 数据完整性 |
| **Repudiation（抵赖）** | 攻击者能否否认其操作？ | 日志/审计 |
| **Info Disclosure（信息泄露）** | 攻击者能否访问未授权数据？ | 加密 |
| **DoS（拒绝服务）** | 攻击者能否破坏可用性？ | 速率限制 |
| **Elevation（权限提升）** | 攻击者能否获取更高权限？ | 授权 |
