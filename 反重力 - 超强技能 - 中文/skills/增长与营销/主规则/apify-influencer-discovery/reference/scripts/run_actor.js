#!/usr/bin/env node
/**
 * Apify Actor Runner - 运行 Apify actors 并导出结果
 *
 * 用法:
 *   # 快速回答（在聊天中显示，不保存文件）
 *   node --env-file=.env scripts/run_actor.js --actor ACTOR_ID --input '{}'
 *
 *   # 导出到文件
 *   node --env-file=.env scripts/run_actor.js --actor ACTOR_ID --input '{}' --output leads.csv --format csv
 */

import { parseArgs } from 'node:util';
import { writeFileSync, statSync } from 'node:fs';

// User-Agent 用于在 Apify 分析中追踪技能使用情况
const USER_AGENT = 'apify-agent-skills/apify-influencer-discovery-1.0.0';

// 解析命令行参数
function parseCliArgs() {
    const options = {
        actor: { type: 'string', short: 'a' },
        input: { type: 'string', short: 'i' },
        output: { type: 'string', short: 'o' },
        format: { type: 'string', short: 'f', default: 'csv' },
        timeout: { type: 'string', short: 't', default: '600' },
        'poll-interval': { type: 'string', default: '5' },
        help: { type: 'boolean', short: 'h' },
    };

    const { values } = parseArgs({ options, allowPositionals: false });

    if (values.help) {
        printHelp();
        process.exit(0);
    }

    if (!values.actor) {
        console.error('错误: --actor 为必填项');
        printHelp();
        process.exit(1);
    }

    if (!values.input) {
        console.error('错误: --input 为必填项');
        printHelp();
        process.exit(1);
    }

    return {
        actor: values.actor,
        input: values.input,
        output: values.output,
        format: values.format || 'csv',
        timeout: parseInt(values.timeout, 10),
        pollInterval: parseInt(values['poll-interval'], 10),
    };
}

function printHelp() {
    console.log(`
Apify Actor Runner - 运行 Apify actors 并导出结果

用法:
  node --env-file=.env scripts/run_actor.js --actor ACTOR_ID --input '{}'

选项:
  --actor, -a       Actor ID（例如 compass/crawler-google-places）[必填]
  --input, -i       Actor 输入，JSON 字符串格式 [必填]
  --output, -o      输出文件路径（可选 - 如不提供则显示快速回答）
  --format, -f      输出格式: csv, json（默认: csv）
  --timeout, -t     最大等待时间，秒（默认: 600）
  --poll-interval   状态检查间隔，秒（默认: 5）
  --help, -h        显示此帮助信息

输出格式:
  JSON（全部数据）     --output file.json --format json
  CSV（全部数据）      --output file.csv --format csv
  快速回答             （无 --output）- 在聊天中显示前 5 条

示例:
  # 快速回答 - 在聊天中显示前 5 条
  node --env-file=.env scripts/run_actor.js \\
    --actor "compass/crawler-google-places" \\
    --input '{"searchStringsArray": ["coffee shops"], "locationQuery": "Seattle, USA"}'

  # 导出全部数据到 CSV
  node --env-file=.env scripts/run_actor.js \\
    --actor "compass/crawler-google-places" \\
    --input '{"searchStringsArray": ["coffee shops"], "locationQuery": "Seattle, USA"}' \\
    --output leads.csv --format csv
`);
}

// 启动 actor 运行并返回 { runId, datasetId }
async function startActor(token, actorId, inputJson) {
    // 将 "author/actor" 格式转换为 "author~actor" 以兼容 API
    const apiActorId = actorId.replace('/', '~');
    const url = `https://api.apify.com/v2/acts/${apiActorId}/runs?token=${encodeURIComponent(token)}`;

    let data;
    try {
        data = JSON.parse(inputJson);
    } catch (e) {
        console.error(`错误: JSON 输入无效: ${e.message}`);
        process.exit(1);
    }

    const response = await fetch(url, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'User-Agent': `${USER_AGENT}/start_actor`,
        },
        body: JSON.stringify(data),
    });

    if (response.status === 404) {
        console.error(`错误: Actor '${actorId}' 未找到`);
        process.exit(1);
    }

    if (!response.ok) {
        const text = await response.text();
        console.error(`错误: API 请求失败 (${response.status}): ${text}`);
        process.exit(1);
    }

    const result = await response.json();
    return {
        runId: result.data.id,
        datasetId: result.data.defaultDatasetId,
    };
}

// 轮询运行状态直到完成或超时
async function pollUntilComplete(token, runId, timeout, interval) {
    const url = `https://api.apify.com/v2/actor-runs/${runId}?token=${encodeURIComponent(token)}`;
    const startTime = Date.now();
    let lastStatus = null;

    while (true) {
        const response = await fetch(url);
        if (!response.ok) {
            const text = await response.text();
            console.error(`错误: 获取运行状态失败: ${text}`);
            process.exit(1);
        }

        const result = await response.json();
        const status = result.data.status;

        // 仅在状态变化时打印
        if (status !== lastStatus) {
            console.log(`状态: ${status}`);
            lastStatus = status;
        }

        if (['SUCCEEDED', 'FAILED', 'ABORTED', 'TIMED-OUT'].includes(status)) {
            return status;
        }

        const elapsed = (Date.now() - startTime) / 1000;
        if (elapsed > timeout) {
            console.error(`警告: ${timeout}秒后超时，actor 仍在运行`);
            return 'TIMED-OUT';
        }

        await sleep(interval * 1000);
    }
}

// 下载数据集项
async function downloadResults(token, datasetId, outputPath, format) {
    const url = `https://api.apify.com/v2/datasets/${datasetId}/items?token=${encodeURIComponent(token)}&format=json`;

    const response = await fetch(url, {
        headers: {
            'User-Agent': `${USER_AGENT}/download_${format}`,
        },
    });

    if (!response.ok) {
        const text = await response.text();
        console.error(`错误: 下载结果失败: ${text}`);
        process.exit(1);
    }

    const data = await response.json();

    if (format === 'json') {
        writeFileSync(outputPath, JSON.stringify(data, null, 2));
    } else {
        // CSV 输出
        if (data.length > 0) {
            const fieldnames = Object.keys(data[0]);
            const csvLines = [fieldnames.join(',')];

            for (const row of data) {
                const values = fieldnames.map((key) => {
                    let value = row[key];

                    // 截断过长的文本字段
                    if (typeof value === 'string' && value.length > 200) {
                        value = value.slice(0, 200) + '...';
                    } else if (Array.isArray(value) || (typeof value === 'object' && value !== null)) {
                        value = JSON.stringify(value) || '';
                    }

                    // CSV 转义: 如包含逗号、引号或换行符则用引号包裹
                    if (value === null || value === undefined) {
                        return '';
                    }
                    const strValue = String(value);
                    if (strValue.includes(',') || strValue.includes('"') || strValue.includes('\n')) {
                        return `"${strValue.replace(/"/g, '""')}"`;
                    }
                    return strValue;
                });
                csvLines.push(values.join(','));
            }

            writeFileSync(outputPath, csvLines.join('\n'));
        } else {
            writeFileSync(outputPath, '');
        }
    }

    console.log(`已保存至: ${outputPath}`);
}

// 在聊天格式中显示前 5 条结果
async function displayQuickAnswer(token, datasetId) {
    const url = `https://api.apify.com/v2/datasets/${datasetId}/items?token=${encodeURIComponent(token)}&format=json`;

    const response = await fetch(url, {
        headers: {
            'User-Agent': `${USER_AGENT}/quick_answer`,
        },
    });

    if (!response.ok) {
        const text = await response.text();
        console.error(`错误: 下载结果失败: ${text}`);
        process.exit(1);
    }

    const data = await response.json();
    const total = data.length;

    if (total === 0) {
        console.log('\n未找到结果。');
        return;
    }

    // 显示前 5 条
    console.log(`\n${'='.repeat(60)}`);
    console.log(`前 5 条结果（共 ${total} 条）`);
    console.log('='.repeat(60));

    for (let i = 0; i < Math.min(5, data.length); i++) {
        const item = data[i];
        console.log(`\n--- 结果 ${i + 1} ---`);

        for (const [key, value] of Object.entries(item)) {
            let displayValue = value;

            // 截断过长的值
            if (typeof value === 'string' && value.length > 100) {
                displayValue = value.slice(0, 100) + '...';
            } else if (Array.isArray(value) || (typeof value === 'object' && value !== null)) {
                const jsonStr = JSON.stringify(value);
                displayValue = jsonStr.length > 100 ? jsonStr.slice(0, 100) + '...' : jsonStr;
            }

            console.log(`  ${key}: ${displayValue}`);
        }
    }

    console.log(`\n${'='.repeat(60)}`);
    if (total > 5) {
        console.log(`显示 ${total} 条结果中的 5 条。`);
    }
    console.log(`完整数据可在以下地址查看: https://console.apify.com/storage/datasets/${datasetId}`);
    console.log('='.repeat(60));
}

// 报告下载数据摘要
function reportSummary(outputPath, format) {
    const stats = statSync(outputPath);
    const size = stats.size;

    let count;
    try {
        const content = require('fs').readFileSync(outputPath, 'utf-8');
        if (format === 'json') {
            const data = JSON.parse(content);
            count = Array.isArray(data) ? data.length : 1;
        } else {
            // CSV - 统计行数减去表头
            const lines = content.split('\n').filter((line) => line.trim());
            count = Math.max(0, lines.length - 1);
        }
    } catch {
        count = '未知';
    }

    console.log(`记录数: ${count}`);
    console.log(`文件大小: ${size.toLocaleString()} 字节`);
}

// 辅助函数: 休眠指定毫秒
function sleep(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}

// 主函数
async function main() {
    // 先解析参数，这样 --help 可以在没有 token 的情况下工作
    const args = parseCliArgs();

    // 检查 APIFY_TOKEN
    const token = process.env.APIFY_TOKEN;
    if (!token) {
        console.error('错误: .env 文件中未找到 APIFY_TOKEN');
        console.error('');
        console.error('请将您的 token 添加到 .env 文件:');
        console.error('  APIFY_TOKEN=your_token_here');
        console.error('');
        console.error('获取 token: https://console.apify.com/account/integrations');
        process.exit(1);
    }

    // 启动 actor 运行
    console.log(`正在启动 actor: ${args.actor}`);
    const { runId, datasetId } = await startActor(token, args.actor, args.input);
    console.log(`运行 ID: ${runId}`);
    console.log(`数据集 ID: ${datasetId}`);

    // 轮询等待完成
    const status = await pollUntilComplete(token, runId, args.timeout, args.pollInterval);

    if (status !== 'SUCCEEDED') {
        console.error(`错误: Actor 运行 ${status}`);
        console.error(`详情: https://console.apify.com/actors/runs/${runId}`);
        process.exit(1);
    }

    // 确定输出模式
    if (args.output) {
        // 文件输出模式
        await downloadResults(token, datasetId, args.output, args.format);
        reportSummary(args.output, args.format);
    } else {
        // 快速回答模式 - 在聊天中显示
        await displayQuickAnswer(token, datasetId);
    }
}

main().catch((err) => {
    console.error(`错误: ${err.message}`);
    process.exit(1);
});
