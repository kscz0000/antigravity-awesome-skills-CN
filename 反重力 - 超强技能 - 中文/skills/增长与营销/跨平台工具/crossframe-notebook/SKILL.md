...
description: "CrossFrame Suite 将明确的笔记任务路由到书籍、理论、文章、摘录、双向阅读、吸收分类或冲突映射时使用。触发词：读书笔记、双向阅读、吸收分类、冲突映射、书籍理论、文章摘录、来源边界"
category: content
risk: safe
source: community
...
tags:
  ...
  - reading
---
# CrossFrame 笔记本（CrossFrame Notebook）—— 读书笔记、双向阅读、吸收分类、冲突映射



## 适用场景

- 涉及 `crossframe-suite` 将明确的 CrossFrame 工作路由到书籍、理论、文章、摘录、双向阅读、吸收分类或冲突映射的笔记时使用。
- 涉及必须把原文本与 CrossFrame 概念区分开时使用。
- 除非用户明确指名调用本兄弟技能，否则不要独立使用。

## 打包来源说明

本 AAS 就绪副本保留了下方原始的 CrossFrame 技能主体。中文是权威语义层；英文元数据仅用于发现、安装和仓库审阅。

## 局限性

- 技能主体刻意以中文为权威；英文元数据仅用于发现，不能替代原有的中文术语。
- 仅在显式 CrossFrame 调用或 `crossframe-suite` 路由之后使用；不要把它当作通用的默认推理层。
- 它能组织分析、起草和审阅，但不能替代来源核验、领域专业知识，也不能替代法律、医疗或财务判断。
