...
name: rclone-flags
title: "Global Flags"
description: "rclone 全局 flag 速查表，按用途分组，覆盖所有 rclone 子命令共用的全局选项（copy、sync、check、filter、listing、logging、metadata、RC、metrics、backend 等）。触发词：rclone 全局参数、rclone flag、rclone 命令行选项、rclone 全局开关、rclone options、rclone global flags、rclone 通用选项"
# autogenerated - DO NOT EDIT
---

> **官方文档：** [https://rclone.org/flags/](https://rclone.org/flags/)
# Global Flags

本文描述每个 rclone 命令都可用的全局 flag，并按用途分组。
...
用于控制 Remote Control API 的 flag。
