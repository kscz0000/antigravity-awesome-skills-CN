---
title: "rclone"
description: 显示 rclone 命令、参数与后端的帮助信息。
...
显示 rclone 命令、参数与后端的帮助信息。

## 概要

Rclone 在云存储服务商之间同步文件，也可以挂载它们，
或以多种不同的方式列出文件。

参见主页（https://rclone.org/）了解安装、使用、
文档、更新日志与配置示例。
...
* [rclone about](/commands/rclone_about/)	 - 获取远端的配额信息。
* [rclone archive](/commands/rclone_archive/)	 - 对归档执行操作。
* [rclone authorize](/commands/rclone_authorize/)	 - 远端授权。
* [rclone backend](/commands/rclone_backend/)	 - 运行后端特定的命令。
* [rclone bisync](/commands/rclone_bisync/)	 - 在两个路径之间执行双向同步。
* [rclone cat](/commands/rclone_cat/)	 - 拼接任意文件并将其发送到标准输出。
* [rclone check](/commands/rclone_check/)	 - 校验源与目标的文件是否一致。
* [rclone checksum](/commands/rclone_checksum/)	 - 对照 SUM 文件校验目标中的文件。
* [rclone cleanup](/commands/rclone_cleanup/)	 - 在可能的情况下清理远端。
* [rclone completion](/commands/rclone_completion/)	 - 为指定 shell 输出补全脚本。
* [rclone config](/commands/rclone_config/)	 - 进入交互式配置会话。
* [rclone convmv](/commands/rclone_convmv/)	 - 原地转换文件与目录的名称。
* [rclone copy](/commands/rclone_copy/)	 - 把文件从源复制到目标，跳过相同的文件。
* [rclone copyto](/commands/rclone_copyto/)	 - 把文件从源复制到目标，跳过相同的文件。
* [rclone copyurl](/commands/rclone_copyurl/)	 - 把所提供 URL 的内容复制到目标路径。
* [rclone cryptcheck](/commands/rclone_cryptcheck/)	 - Cryptcheck 检查加密远端的完整性。
* [rclone cryptdecode](/commands/rclone_cryptdecode/)	 - Cryptdecode 返回解密后的文件名。
* [rclone dedupe](/commands/rclone_dedupe/)	 - 交互式查找重名文件并删除或重命名。
* [rclone delete](/commands/rclone_delete/)	 - 删除路径下的文件。
* [rclone deletefile](/commands/rclone_deletefile/)	 - 从远端删除单个文件。
* [rclone gendocs](/commands/rclone_gendocs/)	 - 将 rclone 的 Markdown 文档输出到指定目录。
* [rclone gitannex](/commands/rclone_gitannex/)	 - 通过标准输入输出与 git-annex 通信。
* [rclone hashsum](/commands/rclone_hashsum/)	 - 为路径下的全部对象生成哈希校验文件。
* [rclone link](/commands/rclone_link/)	 - 为文件或文件夹生成公开链接。
* [rclone listremotes](/commands/rclone_listremotes/)	 - 列出配置文件和环境变量中定义的所有远端。
* [rclone ls](/commands/rclone_ls/)	 - 列出路径中的对象及其大小和路径。
* [rclone lsd](/commands/rclone_lsd/)	 - 列出路径下的全部目录、容器或存储桶。
* [rclone lsf](/commands/rclone_lsf/)	 - 以可解析格式列出远端路径下的目录与对象。
* [rclone lsjson](/commands/rclone_lsjson/)	 - 以 JSON 格式列出路径下的目录与对象。
* [rclone lsl](/commands/rclone_lsl/)	 - 列出路径中的对象，附修改时间、大小和路径。
* [rclone md5sum](/commands/rclone_md5sum/)	 - 为路径下的全部对象生成 md5sum 文件。
* [rclone mkdir](/commands/rclone_mkdir/)	 - 若路径不存在则创建。
* [rclone mount](/commands/rclone_mount/)	 - 将远端作为文件系统挂载到挂载点。
* [rclone move](/commands/rclone_move/)	 - 把文件从源移动到目标。
* [rclone moveto](/commands/rclone_moveto/)	 - 将文件或目录从源移动到目标。
* [rclone ncdu](/commands/rclone_ncdu/)	 - 通过基于文本的界面浏览远端。
* [rclone nfsmount](/commands/rclone_nfsmount/)	 - 将远端作为文件系统挂载到挂载点。
* [rclone obscure](/commands/rclone_obscure/)	 - 为 rclone 配置文件中的密码进行加密混淆。
* [rclone purge](/commands/rclone_purge/)	 - 删除路径及其全部内容。
* [rclone rc](/commands/rclone_rc/)	 - 对运行中的 rclone 执行命令。
* [rclone rcat](/commands/rclone_rcat/)	 - 把标准输入复制到远端的文件。
* [rclone rcd](/commands/rclone_rcd/)	 - 运行 rclone，仅监听远程控制命令。
* [rclone rmdir](/commands/rclone_rmdir/)	 - 删除路径上的空目录。
* [rclone rmdirs](/commands/rclone_rmdirs/)	 - 删除路径下的所有空目录。
* [rclone selfupdate](/commands/rclone_selfupdate/)	 - 更新 rclone 可执行文件。
* [rclone serve](/commands/rclone_serve/)	 - 通过协议提供远端访问。
* [rclone settier](/commands/rclone_settier/)	 - 修改远端对象的存储类别或层级。
* [rclone sha1sum](/commands/rclone_sha1sum/)	 - 为路径下的全部对象生成 sha1sum 文件。
* [rclone size](/commands/rclone_size/)	 - 打印远端路径下的对象总大小与数量。
* [rclone sync](/commands/rclone_sync/)	 - 让源与目标保持一致，仅修改目标。
* [rclone test](/commands/rclone_test/)	 - 运行测试命令
* [rclone touch](/commands/rclone_touch/)	 - 创建新文件或修改文件的修改时间。
* [rclone tree](/commands/rclone_tree/)	 - 以树状结构列出远端的内容。
