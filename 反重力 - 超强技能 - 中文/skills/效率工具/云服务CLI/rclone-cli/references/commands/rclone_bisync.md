...
[Bisync](https://rclone.org/bisync/) 在 rclone 中提供了双向云同步解决方案。
它会保留上一次运行时 Path1 和 Path2 的文件系统列表。
在每次后续运行时，它将：

- 列出 Path1 和 Path2 上的文件，并检查每一端的变更。
  变更包括 `New`、`Newer`、`Older` 和 `Deleted` 文件。
- 将 Path1 上的变更传播到 Path2，反之亦然。

Bisync 被视为一个**高级命令**，因此请谨慎使用。在使用之前，
请务必阅读并理解整个 [manual](https://rclone.org/bisync)
（尤其是 [Limitations](https://rclone.org/bisync/#limitations) 部分），
否则可能会导致数据丢失。如有疑问，可以在
[Rclone Forum](https://forum.rclone.org/) 上提问。

有关详细信息，请参阅 [full bisync description](https://rclone.org/bisync/)。
